...
const botAdmin = new Bot<MyContext>(botTokenAdmin);
...
async function CheckByUSD(conversation: MyConversation, ctx: MyContext) {
  try {
    ...
    if (message.message !== undefined) {
      ...
      try {
        reply = await checkTransactionByUSD(
